...
from case.interface.evopay.QR_single_node_mode.test_exchange_mdaq import Testexchange
...
data_file_mpmqrverification=ReadFile().read_data_file("evopay_evonet_mpmqrverification","QR_single_node_mode","evopay")
mpmqrverification_testdata=ReadCSV(data_file_mpmqrverification).read_data()

data_file_mpmpaymentauthentication=ReadFile().read_data_file("evopay_evonet_mpmpaymentauthentication","QR_single_node_mode","evopay")
mpmpaymentauthentication_testdata=ReadCSV(data_file_mpmpaymentauthentication).read_data()
# #
data_file_paymentnotification=ReadFile().read_data_file("evopay_evonet_paymentnotification","QR_single_node_mode","evopay")
paymentnotification_testdata=ReadCSV(data_file_paymentnotification).read_data()
#
#
data_file_exchange=ReadFile().read_data_file("evopay_evonet_exchange_mdaq","QR_single_node_mode","evopay")
exchange_testdata=ReadCSV(data_file_exchange).read_data()
...
class Test_QR_single_node_mode(object):
    ...
    def evopay_exchange_mdaq1(self, envirs):
        return Testexchange(envirs)

    @pytest.mark.parametrize('test_info', exchange_testdata)
    def test_exchange_mdaq1(self, envirs, test_info):
        self.evopay_exchange_mdaq1(envirs).testexchange(test_info)



    def evopay_exchange_mdaq(self, envirs):
        return Testexchange(envirs)
    submit_test_cases = [
    ...
    def test_exchange_mdaq(self, envirs,title, transtype, transtime, transtypes, transtimes):
        self.evopay_exchange_mdaq(envirs).submit_test_cases(title, transtype, transtime, transtypes, transtimes)
