...
class Testpreorder():
    ...
    def testpreorder(self, test_info):
        result, conf_params, body_params, traceID = self.preorder(test_info)
        self.check_preorder_response(result, test_info, conf_params, body_params, traceID)

    def preorder(self, test_info):
            ...
            if type(body_params.get("fxRate")) == dict:
                ...
                if "?fxrate_value?" == body_params.get("fxRate").get("value"):
                    ...
                    receiveAmount_pre = fxrate_interface.exteral_post_fxrate(head_participant=head_participant,
                                                       ...
                                                       sendCurrency=sendCurrency, receiveCurrency=receiveCurrency)
        ...
        if body_params.get('senderInfo'):
            if type(body_params.get('senderInfo')) == dict:
                ...
                if "?senderInfo.evonetUserReference?" == body_params.get('senderInfo').get('evonetUserReference'):
                    ...
                    account_create_interface_pre = account_create_interface.exteral_post_sop_account_create(head_participant=head_participant,
        ...
        if body_params.get('receiverInfo'):
            if type(body_params.get('receiverInfo')) == dict:
                ...
                if "?receiverInfo.evonetUserReference?" == body_params.get('receiverInfo').get('evonetUserReference'):
                    account_create_interface= every_interface()
                    account_create_interface_pre  = account_create_interface.exteral_post_rop_account_create(head_participant=head_participant,
                                                                                         body_participant=head_participant)
    ...
    def check_preorder_response(self, result, test_info, conf_params, body_params, traceID):
