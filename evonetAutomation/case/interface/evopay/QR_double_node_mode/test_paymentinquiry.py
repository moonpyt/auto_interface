...
data_file=ReadFile().read_data_file("evopay_evonet_paymentinquiry","QR_double_node_mode","evopay")
...
class Testpaymentinquiry():
    ...
    def test_paymentinquiry(self,test_info):
        ...
        if test_info["interface"]=='MPM QR Verification':
            ...
            result = res.json()
            Testpaymentinquiry_double(self.envirs).responce_check_inqiury(result,test_info,traceID,node='double')
        ...
        #payment Inquirey查询接口
            ...
            res = Testpaymentinquiry_double(self.envirs).post_inquiry(test_info, head_params, body_params,node='double')
            headers = res.headers
            traceID = headers['Traceid']
            result = res.json()
            Testpaymentinquiry_double(self.envirs).responce_check_inqiury(result, test_info,traceID,node='double')
