...
from common.evopay.conf_init import evopay_conf, db_tyo_evopay, db_sgp_evologs
...
class Testsettle():
    ...
    def test_settle(self,test_info):
        ...
        #MPM_Payment_Authentication接口
            ...
            # self,method,url,participantID,msgID,datetime,signkey,data
            header = CheckSign().check_sign_post(method=header_method, url=check_sign_url, participantID=participantID,
                                                 msgID=msgID, datetime=datetime, signkey=evopay_conf.signkeyy, data=data)
        ...
        # CPM_payment接口
            ...
            if test_info['pre-update table']:
                if test_info['pre-update mongo']:
                    Update_Mongo_Data(node='double',database=test_info['pre-update database']).updata_data(table=test_info['pre-update table'],
                                                    query_params=eval(test_info['pre-query mongo']),
                                                    update_params=eval(test_info['pre-update mongo']))
                else:

                    Update_Mongo_Data(node='double',database=test_info['pre-update database']).delete_data(table=test_info['pre-update table'],
                                                    query_params=eval(test_info['pre-query mongo']))
            # self,method,url,participantID,msgID,datetime,signkey,data
            header = CheckSign().check_sign_post(method=header_method, url=check_sign_url, participantID=participantID,
                                                 msgID=msgID, datetime=datetime, signkey=evopay_conf.signkey, data=data)
            ...
            traceID = headers['Traceid']
            if test_info['pre-update mongo']:
                Update_Mongo_Data(node='double',database=test_info['pre-update database']).update_data_reset(table=test_info['pre-update table'],
                                                      query_params=eval(test_info['pre-query mongo']),
                                                      update_params=eval(test_info['pre-update mongo']))
            else:
                Update_Mongo_Data(node='double',database=test_info['pre-update database']).delete_data_reset(table=test_info['pre-update table'])
                ...
                if test_info['check_mongo']:
                    ...
                    if interface == 'Refund':
                        if 'payment' in str(test_info['check_mongo']):
                            ...
                            # 校验发送给parnter的数据
                            Check_evonet_to_partner(db_sgp_evologs).check_evonet_to_partner_refund(
